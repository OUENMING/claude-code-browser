...
| 触发条件（分支） | 读哪个文件 | 里面是什么 |
|---|---|---|
| 新增/修改工具契约 | `mcp-server/index.js:310-327` | `TOOLS`——**Claude Code 实际看到的那一份**，工具的单一权威定义 |
| 同上（第二份拷贝，已漂移） | `extension/background.js:25-41` | `TOOL_DEFINITIONS`——同一接口的拷贝，**死数据**，唯一消费者是死分支 `list_tools` |
| 改每次调用的预算 / deadline | `mcp-server/index.js:273-307` ↔ `extension/background.js:117-205` | server 侧 `TOOL_BUDGET_MS` + `budgetFor` + 信封里的 `deadline`；扩展侧 `remainingMs()` / `deadlineError()` / `executeToolBounded` 兜底。**两侧是一对，改一边必须改另一边**（见 T4） |
| 新增工具要挂实现 | `extension/background.js:1402-1419` | `TOOL_HANDLERS`：`name → handler(tabId, args)` |
...
| 跑 deadline 回归测试 | `test/deadline.test.cjs` | 零依赖，29 条断言覆盖 `extension/deadline.js` 的四条不变式（用户代码只被包一次 / 超时走 resolve / 三态可区分 / 探测失败时保守）。`node test/deadline.test.cjs`，退出码 0 = 全过 |
...
**T3 · `stopRequested` 两条路都会复位（2026-09-19 修，之前是单向开关）。** 原来置 `true` 后全仓库无人复位，用户点一次页面上的停止按钮，命令队列就永久死亡，直到重载扩展。现在：`processQueue`（`extension/background.js:145-178`）在队列转为空闲时复位（此刻 stop 已兑现——排队项已丢弃、在飞的工具已返回）；STOP 到达而**没有在飞的活**时立即复位（否则没人会替它复位）。被丢弃的排队调用会收到 `{ code: 'STOPPED' }`，不再让客户端干等传输超时。**碰队列代码时确认这两条复位路径都在**——漏掉任一条就退回原病。
...
上限 60000    ← Claude Code 会把超过 2 分钟的调用转后台，再长就不该指望一次调用跑完
```
...
- `wait_for`（`mcp-server/index.js:287-290`）的预算必须大于它自己的 `timeout` 参数——曾因为两者都等于 30000 而必然失败（见 T5）。
...
**T5 · `wait_for` 曾必然先超时（2026-09-20 修）。** 原来 schema 上限 30000 正好等于传输默认超时 30000 → `wait_for(timeout=30000)` 永远被传输层先判失败。现在它的预算 = `args.timeout + 5000`，页面内轮询再被 `remainingMs()` 压一次。**改任何工具的 timeout 参数上限时，回头看 `budgetFor()`。**
...
2. 动了 `extension/deadline.js`、`handleJavaScript`、`processQueue` 或任何预算常数：`node test/deadline.test.cjs` 全过，退出码 0，**且两侧数字仍满足 T4 的不变式**（扩展 deadline < server 预算 ≤ 60000）。
