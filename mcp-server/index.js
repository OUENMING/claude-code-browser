...
function callExtension(tool, args, budgetMs = budgetFor(tool, args)) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      pending.delete(id);
      reject(new Error(`Timeout calling ${tool}: the extension never answered within ${budgetMs + QUEUE_ALLOWANCE_MS}ms (budget ${budgetMs}ms + ${QUEUE_ALLOWANCE_MS}ms queue allowance). Run health_check to see which hop is stuck.`));
    }, budgetMs + QUEUE_ALLOWANCE_MS);
    pending.set(id, { resolve, reject, timer });
    extWs.send(JSON.stringify({
      type: 'tool_call', id, tool, args,
      deadline: Date.now() + budgetMs - DEADLINE_MARGIN_MS
    }));
  });
}
...
const TOOLS = [
  ...
  { name: 'resolve_actions', description: '把「具名动作」确定性地解析成元素 ref。actions 里每个动作声明 name + 匹配条件（role / name_contains / name_matches / text_contains / text_matches），可选 pick 选择模式（first|last|top 按页面位置|min|max 按数值，配 pick_from 正则，默认抓数字）。返回每个动作的状态：✓ 唯一命中、⚠ 多候选无法唯一确定、✗ 未找到、‼ 参数错误（如正则非法，会附原因）。只读——拿到 ref 后仍需用 computer/form_input 执行。比 find 强在三点：只扫可见可交互元素（不碰折叠菜单和页脚）、能按数值挑「最便宜的票档」、命不中时明说而未找到而不是返回空表。', inputSchema: { type: 'object', properties: { actions: { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, role: { type: ['string', 'array'], items: { type: 'string' } }, name_contains: { type: 'string' }, name_matches: { type: 'string' }, text_contains: { type: 'string' }, text_matches: { type: 'string' }, pick: { type: 'string', enum: ['first', 'last', 'top', 'min', 'max'] }, pick_from: { type: 'string' }, include_disabled: { type: 'boolean' } }, required: ['name'] } }, tabId: { type: 'number' } }, required: ['actions'] } },
